...
class Geocoder
  ...
  KNOWN_PREFIXES = ['FSG','Athletic Team','Free Runners','LV','VC','MR','LR','Laufgruppe','LSV','SV','GS','LCA','SC',
                    'LG','LC','ST','TV','memler.de', 'Migros', 'Lsg', 'Triathlon Club', 'LL', 'Läufergruppe TSV', 'OS',
  'CA', 'Jugendriege', 'KTV', 'Wacker', 'Durchgangszentrum', 'LSG']

  KNOWN_PREFIX_REGEXP = /^(#{KNOWN_PREFIXES.map {|p| Regexp.escape(p) }.join('|')}) /
  def initialize(cache_file, ags_file)
  ...
  def find_lat_long_for(runner)
    if runner.club_or_hometown.blank? or /\d/.match(runner.club_or_hometown) or
        /^[A-Z]{1,3}$/.match(runner.club_or_hometown) or @club_names.include?(runner.club_or_hometown.downcase)
    ...
    unless response[:status] == 'OK'
      ...
      @fail_cache[cache_key] = response[:status]
      return nil, cleaned_hometown
    end

    if response[:results].size > 1 and
        # Accept first result if it's formatted address consists of 'hometown, postcode'
        not (/#{runner.club_or_hometown}, \d+,/.match(response[:results][0][:formatted_address]))
    ...
    return geometry[:location], cleaned_hometown
  end
